...
class X509Util {
    ...
    static byte[] calculateSignature(DERObjectIdentifier dERObjectIdentifier, String str, String str2, PrivateKey privateKey, SecureRandom secureRandom, ASN1Encodable aSN1Encodable) throws IOException, NoSuchProviderException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
    ...
    static byte[] calculateSignature(DERObjectIdentifier dERObjectIdentifier, String str, PrivateKey privateKey, SecureRandom secureRandom, ASN1Encodable aSN1Encodable) throws IOException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        if (dERObjectIdentifier == null) {
            throw new IllegalStateException("no signature algorithm specified");
        }
    ...
    private static RSASSAPSSparams creatPSSParams(AlgorithmIdentifier algorithmIdentifier, int i) {
        return new RSASSAPSSparams(algorithmIdentifier, new AlgorithmIdentifier(PKCSObjectIdentifiers.id_mgf1, (ASN1Encodable) algorithmIdentifier), new ASN1Integer((long) i), new ASN1Integer(1));
    }
    ...
    static DERObjectIdentifier getAlgorithmOID(String str) {
        ...
        return algorithms.containsKey(toUpperCase) ? (DERObjectIdentifier) algorithms.get(toUpperCase) : new DERObjectIdentifier(toUpperCase);
    }
    ...
    static Implementation getImplementation(String str, String str2, Provider provider) throws NoSuchAlgorithmException {
        String toUpperCase = Strings.toUpperCase(str2);
        while (true) {
            String property = provider.getProperty("Alg.Alias." + str + "." + toUpperCase);
            if (property == null) {
                break;
            }
            toUpperCase = property;
        }
        String property2 = provider.getProperty(str + "." + toUpperCase);
        if (property2 != null) {
            try {
                ...
                throw new IllegalStateException("algorithm " + toUpperCase + " in provider " + provider.getName() + " but no class \"" + property2 + "\" found!");
            } catch (Exception e2) {
                throw new IllegalStateException("algorithm " + toUpperCase + " in provider " + provider.getName() + " but class \"" + property2 + "\" inaccessible!");
            }
        }
        throw new NoSuchAlgorithmException("cannot find implementation " + toUpperCase + " for provider " + provider.getName());
    }
    ...
    static AlgorithmIdentifier getSigAlgID(DERObjectIdentifier dERObjectIdentifier, String str) {
        if (noParams.contains(dERObjectIdentifier)) {
            return new AlgorithmIdentifier(dERObjectIdentifier);
        }
        String toUpperCase = Strings.toUpperCase(str);
        return params.containsKey(toUpperCase) ? new AlgorithmIdentifier(dERObjectIdentifier, (ASN1Encodable) params.get(toUpperCase)) : new AlgorithmIdentifier(dERObjectIdentifier, DERNull.INSTANCE);
    }
