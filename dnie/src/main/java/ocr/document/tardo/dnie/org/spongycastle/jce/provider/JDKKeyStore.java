...
public class JDKKeyStore extends KeyStoreSpi implements BCKeyStore {
    ...
    private class StoreEntry {
        ...
        StoreEntry(String alias, Key key, char[] password, Certificate[] certChain) throws Exception {
            ...
            int iterationCount = (JDKKeyStore.this.random.nextInt() & 1023) + 1024;
            ByteArrayOutputStream bOut = new ByteArrayOutputStream();
            DataOutputStream dOut = new DataOutputStream(bOut);
            dOut.writeInt(salt.length);
            dOut.write(salt);
            dOut.writeInt(iterationCount);
            dOut = new DataOutputStream(new CipherOutputStream(dOut, JDKKeyStore.this.makePBECipher(JDKKeyStore.KEY_CIPHER, 1, password, salt, iterationCount)));
        ...
        Object getObject(char[] password) throws NoSuchAlgorithmException, UnrecoverableKeyException {
            ...
            if (this.type == 4) {
                ...
                try {
                    ...
                    dIn.readFully(salt);
                    return JDKKeyStore.this.decodeKey(new DataInputStream(new CipherInputStream(dIn, JDKKeyStore.this.makePBECipher(JDKKeyStore.KEY_CIPHER, 2, password, salt, dIn.readInt()))));
                } catch (Exception e) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                k = JDKKeyStore.this.decodeKey(new DataInputStream(new CipherInputStream(dIn2, JDKKeyStore.this.makePBECipher("BrokenPBEWithSHAAnd3-KeyTripleDES-CBC", 2, password, salt, iterationCount))));
                                dIn = dIn2;
                                byteArrayInputStream = bIn;
                            } catch (Exception e2) {
                                try {
                                    dIn = new DataInputStream(new ByteArrayInputStream((byte[]) this.obj));
                                    salt = new byte[dIn.readInt()];
                                    dIn.readFully(salt);
                                    iterationCount = dIn.readInt();
                                    k = JDKKeyStore.this.decodeKey(new DataInputStream(new CipherInputStream(dIn, JDKKeyStore.this.makePBECipher("OldPBEWithSHAAnd3-KeyTripleDES-CBC", 2, password, salt, iterationCount))));
                                } catch (Exception e3) {
                                    dIn = dIn2;
                            ...
                            if (k != null) {
                                ...
                                dOut.writeInt(iterationCount);
                                dOut = new DataOutputStream(new CipherOutputStream(dOut, JDKKeyStore.this.makePBECipher(JDKKeyStore.KEY_CIPHER, 1, password, salt, iterationCount)));
                                JDKKeyStore.this.encodeKey(k, dOut);
                                dOut.close();
                                this.obj = bOut.toByteArray();
                                return k;
                            }
    ...
    public static class BouncyCastleStore extends JDKKeyStore {
            ...
            if (stream != null) {
                ...
                if (version == 1 || version == 0) {
                    ...
                    if (version == 0) {
                        ...
                        cipherAlg = JDKKeyStore.STORE_CIPHER;
                    }
        ...
        public void engineStore(OutputStream stream, char[] password) throws IOException {
            ...
            dOut.write(salt);
            dOut.writeInt(iterationCount);
            CipherOutputStream cOut = new CipherOutputStream(dOut, makePBECipher(JDKKeyStore.STORE_CIPHER, 1, password, salt, iterationCount));
