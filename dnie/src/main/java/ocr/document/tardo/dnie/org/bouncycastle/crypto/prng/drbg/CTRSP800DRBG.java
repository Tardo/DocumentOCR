...
public class CTRSP800DRBG implements SP80090DRBG {
    ...
    private int getMaxSecurityStrength(BlockCipher blockCipher, int i) {
        return (isTDEA(blockCipher) && i == 168) ? 112 : !blockCipher.getAlgorithmName().equals("AES") ? -1 : i;
    }
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        for (int i = 0; i < bArr.length / obj.length; i++) {
            ...
            System.arraycopy(obj, 0, bArr, obj.length * i, bArr.length - (obj.length * i) > obj.length ? obj.length : bArr.length - (this._V.length * i));
        }
        CTR_DRBG_Update(Block_Cipher_df, this._Key, this._V);
        this._reseedCounter++;
