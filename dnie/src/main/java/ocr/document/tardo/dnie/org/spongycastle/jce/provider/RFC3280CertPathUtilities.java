            ...
            if (idp != null) {
                if (idp.getDistributionPoint() != null) {
                    ...
                    if (dpName.getType() == 1) {
                        ...
                        try {
                            e = ASN1Sequence.getInstance(ASN1Object.fromByteArray(CertPathValidatorUtilities.getIssuerPrincipal(crl).getEncoded())).getObjects();
                    ...
                    if (dp.getDistributionPoint() != null) {
                        ...
                        if (dpName.getType() == 1) {
                            if (dp.getCRLIssuer() != null) {
                                ...
                                try {
                                    genNames[0] = new GeneralName(new X509Name((ASN1Sequence) ASN1Object.fromByteArray(CertPathValidatorUtilities.getEncodedIssuerPrincipal(cert).getEncoded())));
                                } catch (Enumeration e22) {
                                    throw new AnnotatedException("Could not read certificate issuer.", e22);
                                }
                            }
                            for (j = 0; j < genNames.length; j++) {
                                e22 = ASN1Sequence.getInstance(genNames[j].getName().getDERObject()).getObjects();
                                vec = new ASN1EncodableVector();
                                while (e22.hasMoreElements()) {
                                    vec.add((DEREncodable) e22.nextElement());
                                }
                                vec.add(dpName.getName());
            ...
            try {
                ...
                if (deltaCRL.getIssuerX500Principal().equals(completeCRL.getIssuerX500Principal())) {
                    try {
                        ...
                        if (match) {
                            try {
                                DERObject completeKeyIdentifier = CertPathValidatorUtilities.getExtensionValue(completeCRL, AUTHORITY_KEY_IDENTIFIER);
                                try {
                                    DERObject deltaKeyIdentifier = CertPathValidatorUtilities.getExtensionValue(deltaCRL, AUTHORITY_KEY_IDENTIFIER);
                    ...
                    if (policyMapping > 0) {
                        ...
                        if (idp_found) {
                            ...
                            for (PKIXPolicyNode node22 : policyNodes[i]) {
                                if ("2.5.29.32.0".equals(node22.getValidPolicy())) {
                                    Set pq = null;
                                    try {
            ...
            if (pm != null) {
                ...
                while (j < mappings.size()) {
                    try {
                        ...
                        } else if ("2.5.29.32.0".equals(subjectDomainPolicy.getId())) {
    ...
    protected static PKIXPolicyNode processCertD(CertPath certPath, int index, Set acceptablePolicies, PKIXPolicyNode validPolicyTree, List[] policyNodes, int inhibitAnyPolicy) throws CertPathValidatorException {
        ...
        try {
            ...
            if (!acceptablePolicies.isEmpty()) {
                if (!acceptablePolicies.contains("2.5.29.32.0")) {
                    Set t1 = new HashSet();
        ...
        try {
            ...
            if (nc != null) {
                ...
                if (excluded != null) {
                    ...
                    while (e.hasMoreElements()) {
                        try {
                            nameConstraintValidator.addExcludedSubtree(GeneralSubtree.getInstance(e.nextElement()));
                        } catch (Exception ex2) {
        ...
        if (validPolicyTree == null) {
            ...
            for (List _nodeDepth2 : policyNodes) {
                for (k = 0; k < _nodeDepth2.size(); k++) {
                    _node = (PKIXPolicyNode) _nodeDepth2.get(k);
                    if ("2.5.29.32.0".equals(_node.getValidPolicy())) {
                        _iter = _node.getChildren();
                        while (_iter.hasNext()) {
