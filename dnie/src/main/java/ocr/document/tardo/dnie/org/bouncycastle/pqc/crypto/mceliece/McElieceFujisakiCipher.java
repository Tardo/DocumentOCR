...
public class McElieceFujisakiCipher implements MessageEncryptor {
    ...
    private void initCipherEncrypt(McElieceCCA2PublicKeyParameters mcElieceCCA2PublicKeyParameters) {
        ...
        this.f314n = mcElieceCCA2PublicKeyParameters.getN();
        this.f313k = mcElieceCCA2PublicKeyParameters.getK();
        this.f315t = mcElieceCCA2PublicKeyParameters.getT();
    }
    ...
    public void initCipherDecrypt(McElieceCCA2PrivateKeyParameters mcElieceCCA2PrivateKeyParameters) {
        ...
        this.f314n = mcElieceCCA2PrivateKeyParameters.getN();
        this.f315t = mcElieceCCA2PrivateKeyParameters.getT();
    }

    public byte[] messageDecrypt(byte[] bArr) throws Exception {
        int i = (this.f314n + 7) >> 3;
        int length = bArr.length - i;
        byte[][] split = ByteUtils.split(bArr, i);
        byte[] bArr2 = split[0];
        byte[] bArr3 = split[1];
        GF2Vector[] decryptionPrimitive = McElieceCCA2Primitives.decryptionPrimitive((McElieceCCA2PrivateKeyParameters) this.key, GF2Vector.OS2VP(this.f314n, bArr2));
        ...
        if (Conversions.encode(this.f314n, this.f315t, bArr5).equals(obj)) {
            return bArr4;
        }
    ...
    public byte[] messageEncrypt(byte[] bArr) throws Exception {
        GF2Vector gF2Vector = new GF2Vector(this.f313k, this.sr);
        ...
        byte[] encoded2 = McElieceCCA2Primitives.encryptionPrimitive((McElieceCCA2PublicKeyParameters) this.key, gF2Vector, Conversions.encode(this.f314n, this.f315t, concatenate)).getEncoded();
        DigestRandomGenerator digestRandomGenerator = new DigestRandomGenerator(new SHA1Digest());
