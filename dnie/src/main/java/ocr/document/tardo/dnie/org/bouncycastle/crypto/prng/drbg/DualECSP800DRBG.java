...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    public DualECSP800DRBG(Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            if (i <= 128) {
                ...
                this._seedlen = 256;
                this._outlen = 30;
                this._curve = (Fp) NISTNamedCurves.getByName("P-256").getCurve();
                ...
                this._seedlen = 384;
                this._outlen = 46;
                this._curve = (Fp) NISTNamedCurves.getByName("P-384").getCurve();
            ...
            } else if (Utils.getMaxSecurityStrength(digest) < 256) {
                throw new IllegalArgumentException("Requested security strength is not supported by digest");
            } else {
                this._seedlen = 521;
                this._outlen = 63;
                this._curve = (Fp) NISTNamedCurves.getByName("P-521").getCurve();
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr3 != null) {
                bArr3 = Utils.hash_df(this._digest, bArr3, this._seedlen);
            }
            byte[] bArr4 = bArr3;
            int i = 0;
            while (i < length2) {
                this._s = this._P.multiply(new BigInteger(1, xor(this._s, bArr4))).getX().toBigInteger().toByteArray();
                Object toByteArray = this._Q.multiply(new BigInteger(1, this._s)).getX().toBigInteger().toByteArray();
                if (toByteArray.length > this._outlen) {
                    System.arraycopy(toByteArray, toByteArray.length - this._outlen, bArr, this._outlen * i, this._outlen);
                } else {
                    System.arraycopy(toByteArray, 0, bArr, (this._outlen * i) + (this._outlen - toByteArray.length), toByteArray.length);
                }
                this._reseedCounter++;
                i++;
                bArr4 = null;
            }
            if (this._outlen * length2 < bArr.length) {
                this._s = this._P.multiply(new BigInteger(1, xor(this._s, bArr4))).getX().toBigInteger().toByteArray();
                System.arraycopy(this._Q.multiply(new BigInteger(1, this._s)).getX().toBigInteger().toByteArray(), 0, bArr, this._outlen * length2, bArr.length - (length2 * this._outlen));
            }
