...
public class GMSSRootCalc {
    ...
    public GMSSRootCalc(int i, int i2, GMSSDigestProvider gMSSDigestProvider) {
        ...
        this.retain = new Vector[(this.f123K - 1)];
    ...
    public void initialize(Vector vector) {
        int i;
        this.treehash = new Treehash[(this.heightOfTree - this.f123K)];
        for (i = 0; i < this.heightOfTree - this.f123K; i++) {
    ...
    public void update(byte[] bArr) {
        if (this.isFinished) {
            ...
            } else if (this.index[0] == 3 && this.heightOfTree > this.f123K) {
                this.treehash[0].setFirstNode(bArr);
            }
            if ((this.index[0] - 3) % 2 == 0 && this.index[0] >= 3 && this.heightOfTree == this.f123K) {
            ...
            while (this.tailStack.size() > 0 && i == ((Integer) this.heightOfNodes.lastElement()).intValue()) {
                ...
                if (i2 < this.heightOfTree) {
                    ...
                    if (i2 >= this.heightOfTree - this.f123K) {
                        if (i2 == 0) {
                            System.out.println("M���P");
                        }
                        if ((this.index[i2] - 3) % 2 == 0 && this.index[i2] >= 3) {
                            this.retain[i2 - (this.heightOfTree - this.f123K)].insertElementAt(obj3, 0);
                            i = i2;
                        }
    ...
    public void update(byte[] bArr, byte[] bArr2) {
        if (this.heightOfNextSeed < this.heightOfTree - this.f123K && this.indexForNextSeed - 2 == this.index[0]) {
