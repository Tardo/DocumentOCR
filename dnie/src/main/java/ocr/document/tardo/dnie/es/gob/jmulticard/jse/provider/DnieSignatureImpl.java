...
abstract class DnieSignatureImpl extends SignatureSpi {
    ...
    DnieSignatureImpl(String signatureAlgorithm) {
    ...
    protected void engineInitSign(PrivateKey prKey) throws InvalidKeyException {
        ...
        } else if (prKey instanceof DniePrivateKey) {
            this.privateKey = (DniePrivateKey) prKey;
            this.data.reset();
        } else {
            throw new InvalidKeyException("La clave proporcionada no es de un DNIe: " + prKey.getClass().getName());
        }
    }
    ...
    protected byte[] engineSign() throws SignatureException {
        if (this.privateKey.getCryptoCard() instanceof Dnie) {
            try {
                return this.privateKey.getCryptoCard().sign(this.data.toByteArray(), this.signatureAlgo, new DniePrivateKeyReference((Dnie) this.privateKey.getCryptoCard(), this.privateKey.getId(), this.privateKey.getPath(), this.privateKey.toString()));
