...
public class RSAKeyEncapsulation implements KeyEncapsulation {
    ...
    public CipherParameters decrypt(byte[] bArr, int i, int i2, int i3) throws IllegalArgumentException {
        if (this.key.isPrivate()) {
            ...
            this.kdf.init(new KDFParameters(BigIntegers.asUnsignedByteArray((modulus.bitLength() + 7) / 8, new BigInteger(1, obj).modPow(exponent, modulus)), null));
    ...
    public CipherParameters encrypt(byte[] bArr, int i, int i2) throws IllegalArgumentException {
        ...
        byte[] asUnsignedByteArray = BigIntegers.asUnsignedByteArray((modulus.bitLength() + 7) / 8, createRandomInRange);
        Object asUnsignedByteArray2 = BigIntegers.asUnsignedByteArray((modulus.bitLength() + 7) / 8, createRandomInRange.modPow(exponent, modulus));
        System.arraycopy(asUnsignedByteArray2, 0, bArr, i, asUnsignedByteArray2.length);
        this.kdf.init(new KDFParameters(asUnsignedByteArray, null));
