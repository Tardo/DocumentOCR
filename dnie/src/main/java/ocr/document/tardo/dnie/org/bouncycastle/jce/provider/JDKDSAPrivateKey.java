...
public class JDKDSAPrivateKey implements DSAPrivateKey, PKCS12BagAttributeCarrier {
    ...
    JDKDSAPrivateKey(DSAPrivateKey dSAPrivateKey) {
        this.f297x = dSAPrivateKey.getX();
        this.dsaSpec = dSAPrivateKey.getParams();
    }

    JDKDSAPrivateKey(DSAPrivateKeySpec dSAPrivateKeySpec) {
        this.f297x = dSAPrivateKeySpec.getX();
    ...
    JDKDSAPrivateKey(PrivateKeyInfo privateKeyInfo) throws IOException {
        ...
        this.f297x = DERInteger.getInstance(privateKeyInfo.parsePrivateKey()).getValue();
    ...
    JDKDSAPrivateKey(DSAPrivateKeyParameters dSAPrivateKeyParameters) {
        this.f297x = dSAPrivateKeyParameters.getX();
    ...
    private void readObject(ObjectInputStream objectInputStream) throws IOException, ClassNotFoundException {
        this.f297x = (BigInteger) objectInputStream.readObject();
    ...
    private void writeObject(ObjectOutputStream objectOutputStream) throws IOException {
        objectOutputStream.writeObject(this.f297x);
    ...
    public byte[] getEncoded() {
        try {
            return new PrivateKeyInfo(new AlgorithmIdentifier(X9ObjectIdentifiers.id_dsa, new DSAParameter(this.dsaSpec.getP(), this.dsaSpec.getQ(), this.dsaSpec.getG())), new DERInteger(getX())).getEncoded("DER");
        } catch (IOException e) {
            return null;
        }
    }
