...
public class JCEDHPrivateKey implements DHPrivateKey, PKCS12BagAttributeCarrier {
    ...
    JCEDHPrivateKey(DHPrivateKey dHPrivateKey) {
        this.f296x = dHPrivateKey.getX();
        this.dhSpec = dHPrivateKey.getParams();
    }

    JCEDHPrivateKey(DHPrivateKeySpec dHPrivateKeySpec) {
        this.f296x = dHPrivateKeySpec.getX();
    ...
    JCEDHPrivateKey(DHPrivateKeyParameters dHPrivateKeyParameters) {
        this.f296x = dHPrivateKeyParameters.getX();
    ...
    public byte[] getEncoded() {
        try {
            return this.info != null ? this.info.getEncoded("DER") : new PrivateKeyInfo(new AlgorithmIdentifier(PKCSObjectIdentifiers.dhKeyAgreement, new DHParameter(this.dhSpec.getP(), this.dhSpec.getG(), this.dhSpec.getL())), new DERInteger(getX())).getEncoded("DER");
        } catch (IOException e) {
            return null;
        }
    }
