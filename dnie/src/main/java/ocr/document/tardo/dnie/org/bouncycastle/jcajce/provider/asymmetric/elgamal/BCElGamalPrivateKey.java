...
public class BCElGamalPrivateKey implements ElGamalPrivateKey, DHPrivateKey, PKCS12BagAttributeCarrier {
    ...
    /* renamed from: x */
    private BigInteger f509x;

    protected BCElGamalPrivateKey() {
    }

    BCElGamalPrivateKey(DHPrivateKey dHPrivateKey) {
        this.f509x = dHPrivateKey.getX();
    ...
    BCElGamalPrivateKey(DHPrivateKeySpec dHPrivateKeySpec) {
        this.f509x = dHPrivateKeySpec.getX();
    ...
    BCElGamalPrivateKey(PrivateKeyInfo privateKeyInfo) throws IOException {
        ...
        this.f509x = DERInteger.getInstance(privateKeyInfo.parsePrivateKey()).getValue();
    ...
    BCElGamalPrivateKey(ElGamalPrivateKeyParameters elGamalPrivateKeyParameters) {
        this.f509x = elGamalPrivateKeyParameters.getX();
    ...
    public byte[] getEncoded() {
        try {
            return new PrivateKeyInfo(new AlgorithmIdentifier(OIWObjectIdentifiers.elGamalAlgorithm, new ElGamalParameter(this.elSpec.getP(), this.elSpec.getG())), new DERInteger(getX())).getEncoded("DER");
        } catch (IOException e) {
            return null;
        }
    }
