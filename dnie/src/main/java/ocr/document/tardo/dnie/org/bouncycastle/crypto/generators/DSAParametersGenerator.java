...
public class DSAParametersGenerator {
    ...
    private DSAParameters generateParameters_FIPS186_2() {
        ...
        int i = (this.f77L - 1) / CipherSuite.TLS_DH_RSA_WITH_AES_128_GCM_SHA256;
        Object obj3 = new byte[(this.f77L / 8)];
            ...
            while (true) {
                ...
                if (bigInteger.isProbablePrime(this.certainty)) {
                    ...
                    for (int i3 = 0; i3 < 4096; i3++) {
                        ...
                        if (bigInteger2.bitLength() == this.f77L && bigInteger2.isProbablePrime(this.certainty)) {
    ...
    private DSAParameters generateParameters_FIPS186_3() {
        ...
        byte[] bArr = new byte[(this.f78N / 8)];
        int i2 = (this.f77L - 1) / digestSize;
        int i3 = (this.f77L - 1) % digestSize;
        byte[] bArr2 = new byte[digest.getDigestSize()];
        loop0:
        while (true) {
            this.random.nextBytes(bArr);
            hash(digest, bArr, bArr2);
            BigInteger mod = new BigInteger(1, bArr2).mod(ONE.shiftLeft(this.f78N - 1));
            subtract = ONE.shiftLeft(this.f78N - 1).add(mod).add(ONE).subtract(mod.mod(TWO));
            if (subtract.isProbablePrime(this.certainty)) {
                ...
                while (i < i4) {
                    ...
                    mod = bigInteger.add(ONE.shiftLeft(this.f77L - 1));
                    subtract2 = mod.subtract(mod.mod(subtract.shiftLeft(1)).subtract(ONE));
                    if (subtract2.bitLength() == this.f77L && subtract2.isProbablePrime(this.certainty)) {
                        break loop0;
                    }
                    i++;
                }
    ...
    public void init(DSAParameterGenerationParameters dSAParameterGenerationParameters) {
        this.use186_3 = true;
        this.f77L = dSAParameterGenerationParameters.getL();
        this.f78N = dSAParameterGenerationParameters.getN();
        ...
        if (this.f77L < 1024 || this.f77L > 3072 || this.f77L % 1024 != 0) {
            throw new IllegalArgumentException("L values must be between 1024 and 3072 and a multiple of 1024");
        } else if (this.f77L == 1024 && this.f78N != CipherSuite.TLS_DH_RSA_WITH_AES_128_GCM_SHA256) {
            throw new IllegalArgumentException("N must be 160 for L = 1024");
        } else if (this.f77L == 2048 && this.f78N != 224 && this.f78N != 256) {
            throw new IllegalArgumentException("N must be 224 or 256 for L = 2048");
        } else if (this.f77L == 3072 && this.f78N != 256) {
            throw new IllegalArgumentException("N must be 256 for L = 3072");
        } else if (this.digest.getDigestSize() * 8 < this.f78N) {
