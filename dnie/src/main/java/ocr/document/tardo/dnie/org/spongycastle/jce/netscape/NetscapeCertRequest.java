...
public class NetscapeCertRequest extends ASN1Encodable {
    ...
    public NetscapeCertRequest(ASN1Sequence spkac) {
        try {
            ...
            this.sigAlg = new AlgorithmIdentifier((ASN1Sequence) spkac.getObjectAt(1));
            this.sigBits = ((DERBitString) spkac.getObjectAt(2)).getBytes();
            DEREncodable pkac = (ASN1Sequence) spkac.getObjectAt(0);
            if (pkac.size() != 2) {
            ...
            DEREncodable pubkeyinfo = new SubjectPublicKeyInfo((ASN1Sequence) pkac.getObjectAt(0));
            X509EncodedKeySpec xspec = new X509EncodedKeySpec(new DERBitString(pubkeyinfo).getBytes());
            this.keyAlg = pubkeyinfo.getAlgorithmId();
            this.pubkey = KeyFactory.getInstance(this.keyAlg.getObjectId().getId(), BouncyCastleProvider.PROVIDER_NAME).generatePublic(xspec);
        } catch (Exception e) {
    ...
    public boolean verify(String challenge) throws NoSuchAlgorithmException, InvalidKeyException, SignatureException, NoSuchProviderException {
        ...
        Signature sig = Signature.getInstance(this.sigAlg.getObjectId().getId(), BouncyCastleProvider.PROVIDER_NAME);
