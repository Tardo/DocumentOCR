...
public class RainbowSigner implements MessageSigner {
    ...
    private short[] initSign(Layer[] layerArr, short[] sArr) {
        ...
        for (int i = 0; i < layerArr[0].getVi(); i++) {
            this.f331x[i] = (short) this.random.nextInt();
            this.f331x[i] = (short) (this.f331x[i] & 255);
        }
    ...
    public byte[] generateSignature(byte[] bArr) {
        ...
        this.f331x = new short[((RainbowPrivateKeyParameters) this.key).getInvA2().length];
        ...
        do {
            try {
                ...
                while (i < length) {
                    ...
                    sArr2 = this.cf.solveEquation(layers[i].plugInVinegars(this.f331x), sArr2);
                    if (sArr2 == null) {
                        throw new Exception("LES is not solveable!");
                    }
                    for (i2 = 0; i2 < sArr2.length; i2++) {
                        this.f331x[layers[i].getVi() + i2] = sArr2[i2];
                    }
                    i++;
                    i2 = i3;
                }
                sArr = this.cf.multiplyMatrix(((RainbowPrivateKeyParameters) this.key).getInvA2(), this.cf.addVect(((RainbowPrivateKeyParameters) this.key).getB2(), this.f331x));
