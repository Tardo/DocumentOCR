...
public class X509CRLObject extends X509CRL {
    ...
    private Set getExtensionOIDs(boolean z) {
        if (getVersion() == 2) {
            Extensions extensions = this.f95c.getTBSCertList().getExtensions();
            if (extensions != null) {
    ...
    private Set loadCRLEntries() {
        Set hashSet = new HashSet();
        Enumeration revokedCertificateEnumeration = this.f95c.getRevokedCertificateEnumeration();
    ...
    public byte[] getEncoded() throws CRLException {
        try {
            return this.f95c.getEncoded("DER");
        } catch (IOException e) {
    ...
    public Principal getIssuerDN() {
        return new X509Principal(X500Name.getInstance(this.f95c.getIssuer().toASN1Primitive()));
    }

    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f95c.getIssuer().getEncoded());
        } catch (IOException e) {
    ...
    public Date getNextUpdate() {
        return this.f95c.getNextUpdate() != null ? this.f95c.getNextUpdate().getDate() : null;
    }
    ...
    public String getSigAlgOID() {
        return this.f95c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f95c.getSignature().getBytes();
    }

    public byte[] getTBSCertList() throws CRLException {
        try {
            return this.f95c.getTBSCertList().getEncoded("DER");
        } catch (IOException e) {
            throw new CRLException(e.toString());
        }
    }
    ...
    public Date getThisUpdate() {
        return this.f95c.getThisUpdate().getDate();
    }

    public int getVersion() {
        return this.f95c.getVersionNumber();
    }
    ...
    public boolean isRevoked(Certificate certificate) {
        if (certificate.getType().equals("X.509")) {
            CRLEntry[] revokedCertificates = this.f95c.getRevokedCertificates();
            X500Name issuer = this.f95c.getIssuer();
    ...
    public void verify(PublicKey publicKey, String str) throws CRLException, NoSuchAlgorithmException, InvalidKeyException, NoSuchProviderException, SignatureException {
        if (this.f95c.getSignatureAlgorithm().equals(this.f95c.getTBSCertList().getSignature())) {
